...
resource "aws_instance" "tf-ec2" {
  ...
  key_name      = "sam"    # write your pem file without .pem extension>
  tags = {
    "Name" = "tf-ec2"
  }
}
...
resource "aws_s3_bucket" "tf-s3" {
  bucket = "sam-tf-test-bucket-addwhateveryouwant"
}
```
...
resource "aws_s3_bucket" "tf-s3" {
  bucket = "sam-tf-bucket-addwhateveryouwant-new"
  #bucket = "sam-tf-bucket-addwhateveryouwant"
}
```
...
variable "s3_bucket_name" {
  default = "sam-s3-bucket-variable-addwhateveryouwant"
}
...
```go
variable "s3_bucket_name" {
#   default     = "sam-new-s3-bucket-addwhateveryouwant"
}
```
...
```bash
terraform plan -var="s3_bucket_name=sam-new-s3-bucket-2"
```
...
```bash
export TF_VAR_s3_bucket_name=sam-env-varible-bucket
terraform plan
```
...
- Create a file name `sam.tfvars`. Add the followings.

```go
s3_bucket_name = "sam-tfvar-bucket"
```

- Run the command below.

```bash
terraform plan --var-file="sam.tfvars"
```

- Create a file named `sam.auto.tfvars`. Add the followings.

```go
s3_bucket_name = "sam-auto-tfvar-bucket"
```
...
- Terraform loads variables in the following order:
  ...
  - Environment variables
-komut satırında -var kullanıldıysa önce onu dikkate alır. yoksa auto.tfvars ya da auto.tfvars.json dosyalarını dikkate alır. 
onlar yoksa terraform.tfvars.json dikkate alınır. O yoksa terraform.tfvars dikkate alınır. Bunlardan sonra da environment variables dikkate alınır. 
...
resource "aws_s3_bucket" "tf-s3" {
  ...
  bucket   = "sam-tf-s3-bucket-${each.value}"
}
...
```
*** toset: terraform da gömülü bir fonksiyondur, listeyi set e çevirir. 
users variable ını set e çevir diyoruz.  
** var.users içindeki her user için; uzunluğu 6 dan büyük olan isimleri büyük harfle output olarak döndür.
  
for  linki: https://developer.hashicorp.com/terraform/language/expressions/for 
```bash
terraform apply
```
